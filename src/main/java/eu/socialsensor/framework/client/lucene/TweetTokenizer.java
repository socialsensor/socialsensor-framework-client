...
public final class TweetTokenizer extends Tokenizer
{
    ...
    { "<ALPHANUM>", "<APOSTROPHE>", "<ACRONYM>", "<COMPANY>", "<EMAIL>", "<HOST>", "<NUM>", "<CJ>",
            "<ACRONYM_DEP>", "<URL>", "<HASHTAG>", "<MENTION>", "<PERCENTAGE>" };

    private boolean replaceInvalidAcronym;

    private int maxTokenLength = TweetAnalyzer.DEFAULT_MAX_TOKEN_LENGTH;
     ...
     */
    public TweetTokenizer(Version matchVersion, AttributeFactory factory, Reader input)
    {
        super(factory, input);
        this.scanner = new TweetTokenizerImpl(input);
        init(input, matchVersion);
    ...
    public final boolean incrementToken() throws IOException
    {
        ...
        while (true)
        {
            ...
            if (tokenType == TweetTokenizerImpl.YYEOF)
            {
                return false;
            }
            ...
            if (scanner.yylength() <= maxTokenLength)
            {
                ...
                // should
                // remain.
                if (tokenType == TweetTokenizerImpl.ACRONYM_DEP)
                {
                    if (replaceInvalidAcronym)
                    {
                        typeAtt.setType(TweetTokenizerImpl.TOKEN_TYPES[TweetTokenizerImpl.HOST]);
                    ...
                    {
                        typeAtt.setType(TweetTokenizerImpl.TOKEN_TYPES[TweetTokenizerImpl.ACRONYM]);
                    }
                }
                else
                {
                    typeAtt.setType(TweetTokenizerImpl.TOKEN_TYPES[tokenType]);
                }
                if(tokensList.size()>0)
                    this.restoreState(tokensList.remove());
                /**
